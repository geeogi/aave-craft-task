...
v4_current AS (
    SELECT
        user,
        SUM(current_position_usd) AS current_v4_balance_usd
    FROM query_7676585
    GROUP BY user
),
v3_current AS (
...
migrators AS (
    SELECT
        ...
        COALESCE(v4_current.current_v4_balance_usd, 0) AS current_v4_balance_usd,
        COALESCE(v3_current.current_v3_balance_usd, 0) + COALESCE(v4_current.current_v4_balance_usd, 0) AS current_total_balance_usd
    FROM v3_start_balances
    INNER JOIN v4_users
        ON v3_start_balances.user = v4_users.user
    LEFT JOIN v4_current
        ON v3_start_balances.user = v4_current.user
...
classified AS (
    SELECT
        ...
        current_v4_balance_usd,
        current_total_balance_usd,
        CASE
            WHEN current_total_balance_usd < 1e3 THEN '1-1k'
            WHEN current_total_balance_usd < 1e4 THEN '1k-10k'
            WHEN current_total_balance_usd < 1e5 THEN '10k-100k'
            WHEN current_total_balance_usd < 1e6 THEN '100k-1m'
            WHEN current_total_balance_usd < 1e7 THEN '1m-10m'
            WHEN current_total_balance_usd < 1e8 THEN '10m-100m'
            ELSE '100m+'
        END AS balance_bucket,
        CASE
            WHEN current_total_balance_usd < 1e3 THEN 1
            WHEN current_total_balance_usd < 1e4 THEN 2
            WHEN current_total_balance_usd < 1e5 THEN 3
            WHEN current_total_balance_usd < 1e6 THEN 4
            WHEN current_total_balance_usd < 1e7 THEN 5
            WHEN current_total_balance_usd < 1e8 THEN 6
            ELSE 7
        END AS bucket_order
    FROM migrators
),
bucket_aggregates AS (
    SELECT
        balance_bucket,
        bucket_order,
        SUM(current_total_balance_usd) AS total_balance_usd,
        SUM(current_v3_balance_usd) AS current_v3_balance_usd,
        SUM(current_v4_balance_usd) AS current_v4_balance_usd
    FROM classified
    GROUP BY
        balance_bucket,
        bucket_order
...
SELECT
    ...
    COALESCE(bucket_aggregates.total_balance_usd, 0) AS total_balance_usd,
    COALESCE(bucket_aggregates.current_v3_balance_usd, 0) AS current_v3_balance_usd,
    COALESCE(bucket_aggregates.current_v4_balance_usd, 0) AS current_v4_balance_usd
FROM bucket_defs
